...
小さく静かに動くことを目指して設計されています — Dock アイコンなし、単一のポップオーバー、インクリメンタルログスキャン、小さなメモリフットプリント。
...
- 単一のポップオーバー、Dock アイコンなし、分析やテレメトリなしの **ミニマルデザイン**。
- 小さなメモリ常駐量と低いアイドル CPU、ユーザー設定のリフレッシュ間隔の間はスリープする **軽量フットプリント**。
